...
knitr::opts_knit$set(root.dir = paste0(ROOT, "/Report"))
# load libraries
library(xlsx)
library(ggplot2)
library(dplyr)
...
all_df <- read.xlsx(paste0(ROOT, "/All_Transactions.xlsx"), 1, header=TRUE)
...
reimbursed <- df %>%
                  filter(Category == "Reimbursed") %>%
                    select(Credit, Debit) %>%
                      pivot_longer(cols = 1:2, names_to = "type", values_to = "Amount")
excess_reimbursement <- sum(reimbursed$Amount)
excluded_df <- df %>%
                filter(Category == "Miscellaneous") %>%
                  filter(grepl("Ebi|Helping Lakshya", Subcategory))
excluded_df <- rbind(excluded_df, filter(excluded_df, Category == "Reimbursed"))
excluded_df <- rbind(excluded_df, filter(excluded_df, Category == "CreditCardPayment"))
df <- df %>%
        filter(!Category == "Miscellaneous") %>%
        filter(!Category == "Reimbursed") %>%
        filter(!Category == "CreditCardPayment") %>%
          filter(!grepl("Ebi|Helping Lakshya", Subcategory))
all_df <- all_df %>%
        filter(!Category == "Miscellaneous") %>%
        filter(!Category == "Reimbursed") %>%
...
observe({
    input$Last_year1
    updateDateRangeInput(session, "date_range", "Date range:",
      start = paste0(as.numeric(format(Sys.Date(), "%Y"))-1, "-01-01"),
...
#         geom_bar(stat="sum", fill=brewer.pal(11, "Spectral")[1]) +
#           ylab("Amount in CHF") +
#             xlab("Month") +
#               make_theme(theme_few(), leg_pos="none", setFill=F,
#                          x_angle=70, x_size=15, x_hj=1, x_vj=1,
#                          y_size=15
#               )
#   p
...
#         ylab("Amount in CHF") +
#           xlab("Month") +
#             make_theme(max_colors=length(unique(df_plot$Category)), palettefill = "Dark2",
#                         guide_nrow = round(length(unique(df_plot$Category))/2),
#                         x_angle=70, x_size=10, x_hj=1, x_vj=1,
...
#         ylab("Amount in CHF") +
#           xlab("Month") +
#             make_theme(max_colors = length(unique(df_plot$Category)), palettefill = "Accent",
#                         guide_nrow = round(length(unique(df_plot$Category))/2),
#                         x_angle=70, x_size=10, x_hj=1, x_vj=1,
#                         guide_nrow_byrow = T, leg_pos = "bottom")
#
...
#           xlab("Month") +
#             make_theme(x_angle=70, x_size=10, x_hj=1, x_vj=1,
#                        leg_pos="none"
#             )
#
# p1
...
Credit card transactions are included in the source data so transaction towrds credit card payment are excluded from reports (to avoid duplication of credit card expenses).

There is an excess of **CHF `r excess_reimbursement`** from the reimbursements because part of the money that was reimbursed was paid from indian credit cards not associated with my current account.
...
In the future, if particular Miscellaneous transactions are to be excluded, they need to be mentioned seperately (i.e. the code excluding transactions should be modified to consider them). Any transactions flagged as reimbursed will be excluded (even if the reimbursement has not been credited yet!)
